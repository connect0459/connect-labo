...
- **共有予定ロジックの性質**: 「Chicory/WasmKitの成熟度」と「性能要件を満たすか」は独立した命題である。前者はツールチェーンの機能対応、後者は実行するワークロードの特性（CPUバウンドな計算か、単なるルーティン処理か）に依存する。経路Aが第一段階として動作している現状では優先度は下がるが、経路Aの実機展開（NDK/iOSクロスコンパイル）で性能上の課題が出た場合や、経路Bの前提（WasmKitのGC対応）が動き出した場合には、この論点が再び「経路Bを選ぶかどうか」の判断に直接効いてくる。
