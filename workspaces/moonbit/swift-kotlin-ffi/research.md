...
- README.mdに明記されている決定的事実：
  ...
  この事実により、「nativeバックエンド経由でexport方向のシンボル命名規則が安定しているか」という問いは、ソースを読んで検証することが原理的にできないと判明した。技術的成熟度の問題ではなく、可観測性・ガバナンスの問題である。
...
- **共有予定ロジックの性質のヒアリング（優先度高、未着手）**: 経路Aが同一ホストでの実証からiOS Simulator・Androidエミュレータでの実機展開まで進んだことで、後回しにできない設計判断が具体化してきた。(a) マーシャリング設計の粒度（複合型・ネストしたコレクションをopaqueハンドル1個にまとめるか、フィールドごとにアクセサを分けるか）、(b) FFI呼び出しオーバーヘッドの許容度（高頻度呼び出しが想定されるならJNI/Cモジュールマップ境界のコストが無視できなくなる）、(c) この境界を作るだけの複雑さがロジックにあるか（単純なバリデーション程度なら見合わないコストになりうる）——いずれも共有予定ロジックの具体像（種類・計算量のオーダー・呼び出し頻度）が分からないと判断できない。**この論点は経路Bの採否とは独立であり、WasmKit/Chicoryの対応状況（下記「経路B関連」節）とは無関係に、経路A自身の実装設計として着手すべきもの。**

### 経路B関連（低優先度・定点観測。経路Aを第一段階として採用済みのため、状況が変わらない限り着手しない）

2026-07-31時点の調査で「WasmKit（Swift側）はWasm GC提案が未実装、iOS側で経路Bは原理的に成立しない」と判定し、経路Aを第一段階として選んだ経緯がある。以下はこの判定が古くなっていないかを再確認するための定点観測項目であり、「未着手のタスク」ではなく「変化が起きるまで放置してよい監視対象」という位置づけである。**2026-08-01に再調査したが、状況に変化はなく、経路B（特にiOS/Swift側）がブロックされたままであるという判定は変わらない。**

- ~~WasmKitのWasm GC提案着手の見込み・時期~~ → **2026-08-01再確認：変化なし。** READMEの対応表は依然「Garbage Collection: ❌ Not implemented」。GC関連のIssue/PRを検索したが新規のものは0件で、着手の兆候は見られない。定期的な再確認を継続する。
- ~~ChicoryのAOTコンパイラエンジンでのWasm GC対応有無~~ → **2026-08-01再確認：未確認のまま変化なし。** README上「AOT engine passes all the same specs as interpreter」というチェック済み項目があり紛らわしいが、これは2024年（GC提案実装より前）のマイルストーンであり、AOT側のWasmGC対応を意味しない。AOT+GCを直接扱うIssue/PRも見つからなかった。
- ~~Chicory Issue [#1216](https://github.com/dylibso/chicory/issues/1216)（JVM GCとの統合）の解消見込み~~ → **2026-08-01再確認：依然OPEN、進展なし**（最終コメントは2026-04-23のまま）。最新リリースも1.7.5（2026-03-24）から更新されていない。
...
解決・確認済みとなった残課題の記録。問いと結論の対応が追えるよう、当初の問いを残したまま矢印で結果を付記する形式を保っている。
...
- ~~手動リンクの再現性・保守性の評価~~ → **2026-08-01、`spike-native-export/scripts/build-native-lib.sh` として一部解決。** 手動リンク手順（`moon build --target native` → `export_spike.o` / `runtime.o` / `~/.moon/lib` 配下の補助オブジェクトを集めて `cc -shared`）をスクリプト化し、7つの既存スパイク（Java/Kotlin JNI・Swift・複合型・ライフタイム・解放実装検証の各バリアント）すべてで再ビルド→動作確認まで通ることを実測した。スクリプトは `moon version` / `moonc -v` を最後に検証したバージョンと比較し、不一致時は警告のみ出す（ハードフェイルにはしない）。**ただしこれは「バージョンドリフトを検知しやすくした」だけであり、`~/.moon/lib` のファイルレイアウトが将来変わった場合に自動追従する仕組みではない。** また、このスクリプトは native ターゲットの「新戦略」（直接マシンコード生成）のみを前提にしており、後述のCバックエンド戦略（Android/iOS展開に使う方）には未対応だったが、これも後述の通り別スクリプトで解決した。
- ~~リリースビルドでの再現確認~~ → **2026-08-01、当初の問い自体が的外れだったと判明。** `moon build --target native --release` は「新戦略」ではなく別のコード生成戦略（Cバックエンド、`.c`ソースを出力）を使うことが判明したため、「同じ手動リンク手順が通るか」という問いは成立しない。releaseビルドはCバックエンド経由でAndroid/iOS展開に使う前提で別途スクリプト化した（次項）。
...
- ~~iOS実機/シミュレータ向けの確認~~ → **2026-08-01確認済み（訂正後）。当初「再リンクは実測でブロックされる」としたのは、精製済みオブジェクト（新戦略の`.o`）を再リンクする迂回策の話であり、Cバックエンド戦略（`.c`ソースからのビルド）では別物としてiOS Simulator上での実行まで確認できた。** 詳細は上記「訂正：moonc の『Cバックエンド』経由でAndroid/iOSともに実機で動作した」節を参照。iOS**実デバイス**（Simulatorではなく）での確認は残課題として上に記載している。
